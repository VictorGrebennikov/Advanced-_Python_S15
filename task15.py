...
logging.basicConfig(filename='loggingERROR_creat_person.log', level=logging.ERROR, encoding='utf-8')
...
def main():
    ...
    parser.add_argument('-ln', '--lastname', help='Фамилия', required=True)
    parser.add_argument('-n', '--name', help='Имя', required=True)
    parser.add_argument('-sn', '--surname', help='Отчество', required=True)
    parser.add_argument('-a', '--age', help='Возраст', required=True, type=int)
    parser.add_argument('-id', '--id', help='Идентификационный номер (только для Employee)', type=int)
